...
class usersUsersModel extends usersUsersModel_Parent
{
    ...
    public function getUsers ($id_parent = null, $max_depth = 0, $min_depth = 0)
    {
        $db = $this->getModel('db');
        if (!$id_parent) {
            $sql = "SELECT `id`, `login`, 0 AS `depth`, `active`
                      FROM `" . $this->table_users . "` ";
        } else {
            $sql = "SELECT `id`, `login`, `depth`, `active`
                      FROM `" . $this->table_users . "`
    ...
    public function modUser ($id, $donnees)
    {
        ...
        if ($user) {
            ...
            if ($user) {
                ...
                if ((isset($parent_direct['key']) && $parent_direct['key'] != $user['id_parent']) || (!isset($parent_direct['key']) && $user['id_parent'])) {
