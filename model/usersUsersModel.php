...
class usersUsersModel extends usersUsersModel_Parent
{
    ...
    {
        $ns = $this->getModel('fonctions');
        if (method_exists(get_parent_class(get_parent_class($this)), 'sanitize')) {
            $secure_array = parent::sanitize($insecure_array);
        }
        if (!isset($secure_array) || !is_array($secure_array)) {
            $secure_array = array();
        }
    ...
    public function validate($donnees, $id = null)
    {
        ...
        if (method_exists(get_parent_class(get_parent_class($this)), 'validate')) {
            parent::validate($donnees, $id);
        }
