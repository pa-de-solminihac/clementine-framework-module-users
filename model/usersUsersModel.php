...
class usersUsersModel extends usersUsersModel_Parent
{
    ...
    /**
     * addGroup : cree un nouveau groupe et renvoie son id
     * 
     * @param mixed $name
     * @access public
     * @return void
    ...
    public function addGroup ($name, $id_parent = null)
    {
        ...
        if (!$group) {
            ...
            $sql  = "INSERT INTO " . $this->table_groups . " (`group`, `id_parent`) VALUES ('" . $db->escape_string($name) . "', '" . (int) $id_parent . "')";
            ...
            if ($group) {
                ...
                $sql  = "UPDATE " . $this->table_groups . "
                            SET `group`             = '" . $db->escape_string($group['group']) . "',
                                `id_parent`         = '" . $db->escape_string($group['id_parent']) . "'
                          WHERE `id` = '$id'
                          LIMIT 1 ";
                $stmt = $db->query($sql);
