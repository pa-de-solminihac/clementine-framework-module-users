...
class usersUsersController extends usersUsersController_Parent
{
        ...
        if (!empty($_POST)) {
            ...
            if ($user) {
                ...
                $html_body  = "
                    ...
                    <br />
                    <hr />
                    Note : cet e-mail a été envoyé automatiquement suite à une demande reçue sur notre site. Merci de ne pas y répondre. ";
                // version texte
    ...
    function validuserAction ($params = null)
    {
        ...
        if (count($erreurs)) {
            ...
            if (!$id) {
                $this->data['message'] = 'Cet utilisateur existe déjà';
            } else {
                $olduser = $users->modUser($id, $donnees);
                if (!$olduser) {
                    $this->data['message'] = 'Impossible de modifier cet utilisateur';
                } else {
                    $ret['user'] = $olduser;
                }
            }
